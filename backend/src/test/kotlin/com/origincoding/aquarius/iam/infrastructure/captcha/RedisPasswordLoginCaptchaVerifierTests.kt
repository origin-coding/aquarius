...
class RedisPasswordLoginCaptchaVerifierTests {
    ...
    @Test
    fun `verify rejects correct code for different login name`() {
        `when`(captchaStore.challengeBucket(anyString())).thenReturn(challengeBucket)
        `when`(challengeBucket.get()).thenReturn(record(code = "1234", loginName = "alice"))

        assertThrows<InvalidCaptchaException> {
            verifier.verify(command(code = "1234", loginName = "bob"))
        }
    ...
    @Test
    fun `verify rejects legacy challenge without target hash and deletes redis record`() {
        `when`(captchaStore.challengeBucket(anyString())).thenReturn(challengeBucket)
        `when`(challengeBucket.get()).thenReturn(record(code = "1234", targetHash = null))
    ...
    private fun record(
        code: String,
        loginName: String = "alice",
        targetHash: String? = PasswordLoginCaptchaTargetHasher()
            .hash(DefaultLoginNameNormalizer().normalize(loginName)!!),
        attemptCount: Int = 0,
