...
class IamAccessDeniedHandler(
    ...
    override fun handle(
        ...
        response.status = resultCodeHttpStatusRegistry.getStatus(IamAuthResultCode.ACCESS_DENIED).value()
        response.contentType = MediaType.APPLICATION_JSON_VALUE
        response.characterEncoding = StandardCharsets.UTF_8.name()
        jsonMapper.writeValue(response.outputStream, JsonResponse.error(IamAuthResultCode.ACCESS_DENIED))
    }
}
